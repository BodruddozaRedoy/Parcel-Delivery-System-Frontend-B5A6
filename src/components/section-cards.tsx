...
export function SectionCards(data: { data: Parcel[] }) {
  ...
  const deliveredParcels = data?.data?.filter((p: Parcel) => p.currentStatus === "delivered")
  const inTransitParcels = data?.data?.filter((p: Parcel) => p.currentStatus === "in_transit")
  const canceledParcels = data?.data?.filter((p: Parcel) => p.currentStatus === "canceled")
